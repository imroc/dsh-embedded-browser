 ...
 * `browser_embedded_emulate` takes the *same* device preset names as its
 * `browser_*` counterpart for exactly that reason.
 *
...
import { DEVICE_NAMES } from './browser.js'
...
export function defineTools({ manager, human, hub, config, getAttachments, logger, getPanelAvailable = () => true }) {
  ...
  const emulate = defineTool({
    name: 'browser_embedded_emulate',
    description:
      "Change what this session's page believes about its environment, so a page can be checked at a phone width or in the dark before you screenshot it. Pass a `device` preset (the same names the browser_* channel uses), or an explicit `width`+`height`, and/or `theme` to force the light or dark colour scheme. The overrides apply to this session's tab only — other sessions and the Chrome process are untouched, and no restart is involved — and they are not inherited: `navigate` with newTab drops back to the default viewport. Use reset: true to restore the default environment.",
    parameters: {
      device: { type: 'string', enum: DEVICE_NAMES, description: 'Device preset: viewport, pixel ratio, user agent and touch together.' },
      width: { type: 'number', description: 'Viewport width in CSS pixels (with height; only when no device is given).' },
      height: { type: 'number', description: 'Viewport height in CSS pixels (with width; only when no device is given).' },
      scale: { type: 'number', description: 'Device pixel ratio for an explicit width/height (default 1).' },
      mobile: { type: 'boolean', description: 'Emulate a mobile viewport for an explicit width/height (default false).' },
      touch: { type: 'boolean', description: 'Emulate touch input; defaults to the same value as mobile.' },
      theme: { type: 'string', enum: ['light', 'dark', 'system'], description: "Force the page's prefers-color-scheme; system clears the override." },
      reset: { type: 'boolean', description: 'Restore the default viewport, user agent, touch state and colour scheme. Cannot be combined with anything else.' },
    },
    output: jsonOutput(),
    execute: async (args, exec) => {
      const status = await manager.emulate(sessionOf(exec), args)
      return clean({ viewport: status.session?.viewport ?? null, emulation: status.session?.emulation ?? null, url: status.session?.url ?? null })
    },
  })

  const evaluate = defineTool({
    name: 'browser_embedded_eval',
    description:
      "Evaluate a JavaScript expression in this session's page and return the result as JSON — for switching a theme attribute, reading computed styles, or asserting on state before you screenshot. Runs in this session's own tab, in the page's origin, and cannot reach another origin or another session. The result is a bounded JSON projection, so a DOM node comes back as '[Node <div>]' and a circular structure as '[Circular]' instead of failing.",
    parameters: {
      expression: { type: 'string', required: true, description: 'JavaScript evaluated as a script in the page, e.g. document.title or getComputedStyle(document.body).backgroundColor.' },
    },
    output: jsonOutput(),
    execute: async (args, exec) => clean(await manager.evalInPage(sessionOf(exec), String(args.expression))),
  })
  ...
  return [status, navigate, snapshot, click, type, press, scroll, emulate, evaluate, screenshot, askHuman, close]
}
