...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    /** Auto-open poll: fast enough that a hand-over request is not missed. */
    const REVEAL_POLL_MS = 2000
    /** Auto-open poll while the document is hidden: nothing is being watched. */
    const REVEAL_IDLE_POLL_MS = 15000
    ...
    function apply(ctx) {
      ...
      //
      // Every seat here can be absent on an older or headless client, and a
      // browser-only plugin must never take the workbench down with it: each
      // registration is feature-checked and the tabs are simply missing when the
      // Sidebar is not there.
      ...
      if (tabs !== undefined && typeof tabs.register === 'function') {
        ...
        console.warn('[dsh-embedded-browser] no sidebarRightTabs service: the browser tab cannot be registered')
      }
      ...
      ctx.effect(() => watchSessions(ctx), 'embedded-browser: tab reveal watcher')
    }
